...
def run_bertopic_low_noise_params(texts, embeddings, run_name, K):
    print(f"\n--- Running BERTopic (Tweak Params): {run_name} ---")
    
    #  clusterer more "forgiving"
    # min_cluster_size: smaller values = more topics, less noise
    # min_samples: smaller values = points are more easily included in clusters
    ...
    topic_model = BERTopic(
        hdbscan_model=hdbscan_model, # Use custom clusterer
        nr_topics=K,
    ...
    for topic_id in topic_info.Topic:
        if topic_id != -1:
            ...
            if len(words) >= 5:  # skip degenerate topics
...
for K in K_VALUES:
    ...
    run_bertopic_low_noise_params(heavy_texts, embeddings_light, f"LIGHT_EMBfinal_noiseparams_fullK{K}", K)
    run_bertopic_low_noise_params(heavy_texts, embeddings_heavy, f"HEAVY_EMBfinal_noiseparams_fullK{K}", K)
    run_bertopic_low_noise_params(heavy_texts, embeddings_light, f"LIGHT_EMBfinal2_noiseparams_fullK{K}", K)
    run_bertopic_low_noise_params(heavy_texts, embeddings_heavy, f"HEAVY_EMBfinal2_noiseparams_fullK{K}", K)

    run_bertopic_low_noise_params(heavy_texts, embeddings_light, f"LIGHT_EMBfinal3_noiseparams_fullK{K}", K)
    run_bertopic_low_noise_params(heavy_texts, embeddings_heavy, f"HEAVY_EMBfinal3_noiseparams_fullK{K}", K)
